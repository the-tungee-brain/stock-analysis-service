...
def _max_universe() -> int:
    return int(os.environ.get("EMERGING_LEADERS_MAX_UNIVERSE", "350"))
...
def _score_symbol(symbol: str) -> EmergingLeaderEvaluation | None:
    try:
        ...
        return evaluate_emerging_leader(symbol, raw)
...
def build_emerging_leaders(*, limit: int = 20) -> EmergingLeadersResponse:
    ...
    max_scan = _max_universe()
    scan_symbols = universe[:max_scan]
    ...
    candidates = [s for s in scan_symbols if s.upper() not in top_mover_symbols]
